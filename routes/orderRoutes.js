...
router.post("/", optionalAuthenticate, async (req, res, next) => {
  try {
    ...
    const map = new Map(products.map(p => [p._id.toString(), p])); const orderItems = []; let subtotal = 0;
    for (const requested of items) {
      const product = map.get(String(requested.productId)); const quantity = Number(requested.quantity);
      ...
      const unitPrice = variant ? variant.price : product.basePrice; const totalPrice = unitPrice * quantity; subtotal += totalPrice;
    ...
    const shippingFee = subtotal >= 2000 ? 0 : 99; const tax = Math.round(subtotal * .18 * 100) / 100;
    const order = await Order.create({ orderNumber: generateOrderNumber(), user: req.user?._id || null, customer: { name, email, phone: phone || null }, items: orderItems, shippingAddress, subtotal, shippingFee, tax, total: subtotal + shippingFee + tax, payment: { method: paymentMethod, status: paymentMethod === "cod" ? "pending" : "processing" } });
...
router.get("/:id", optionalAuthenticate, async (req, res, next) => { try { const order = await Order.findById(req.params.id); if (!order) return res.status(404).json({ success: false, message: "Order not found" }); if (order.user && (!req.user || order.user.toString() !== req.user._id.toString()) && req.user?.role !== "admin") return res.status(403).json({ success: false, message: "Order access denied" }); res.json({ success: true, order }); } catch (e) { next(e); } });
