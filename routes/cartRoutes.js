...
async function loadCart(req, res) {
  const filter = req.user ? { user: req.user._id } : { sessionId: sessionId(req, res) };
  let cart = await Cart.findOne(filter).populate("items.product");
  if (!cart) cart = await Cart.create(req.user ? { user: req.user._id } : { sessionId: filter.sessionId });
  return cart;
}
...
router.post("/items", async (req, res, next) => {
  try {
    ...
    if (variantId && !product.variants.id(variantId)?.isActive) return res.status(400).json({ success: false, message: "Invalid variant" });
